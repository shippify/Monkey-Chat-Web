...
window.initChat = function(user, callback){
	...
	monkey.init(vars.MONKEY_APP_ID, vars.MONKEY_APP_KEY, user,null,false, vars.MONKEY_DEBUG_MODE, false,true, false,function(error,success){
		...
		let updatedMetadata=Object.assign({},user)
...
class MonkeyChat extends Component {
	...
	handleConversationOpened(conversation) {
		...
		console.log("handle conversation open")
...
monkey.on('StatusChange', function(status){
	...
	try{
		...
		console.log("Error Monkey Chat Instance: ",exception);
		// tryo to reconnect
	}
...
function defineConversation(conversationId, mokMessage, name, urlAvatar){
	...
	if(message){
		if (isConversationGroup(conversation.id) && store.getState().users[message.senderId].name) {
...
function isOpenBalloon(){
	...
	const element=document.getElementsByClassName("mky-wrapper-out mky-partialsize mky-rightside animated pulse")
	if(!element){
		return  true
	}
	const baloon = element[0];
